...
if cfg.cnn_model:  # path to model was defined
    anomaly_model_name, _ = os.path.splitext(cfg.cnn_model)
    model_path, anomaly_model_name = os.path.split(anomaly_model_name)

    output_path = os.path.join(model_path, cfg.anomaly_model_folder + "_" + anomaly_model_name)

    anomaly_model_path = os.path.join(output_path, anomaly_model_name + '.pkl')
else: # path for model, use pre-trained standard model
    anomaly_model_name = cfg.model_architecture + '_backbone'
    model_path = cfg.code_output_path + string_time
...
if not cfg.cnn_model: # save backbone model
    saved_model_path_full = os.path.join(model_path, anomaly_model_name + '.pt')
...
save_configuration(cfg, os.path.join(output_path, 'anomaly_detector_configuration.txt'))
...
if normal_test_data_loader:
    ...
    scores_normal_test, normal_test_im_names, normal_test_labels = anomaly_detection(clf, model, normal_test_data_loader, dev, feat_container=features_to_visualize, cache_path=output_path)
    if scores_normal_test is not None:
        scores.extend(scores_normal_test)
        save_to_excel(scores_normal_test, normal_test_im_names, normal_test_labels, os.path.join(output_path, cfg.csv_liver_tissue_testnormals))
        save_images_from_excel(os.path.join(output_path, cfg.csv_liver_tissue_testnormals), output_path, cfg.save_n_FP, 'FP')

if liver_anomaly_test_data_loader:
    logging.info('-----------anomaly detection in liver with conditions----------------')
    scores_liver_anomaly_test, liver_anomaly_test_im_names, liver_anomaly_test_labels = anomaly_detection(clf, model, liver_anomaly_test_data_loader, dev, feat_container=features_to_visualize, cache_path=output_path)
    if scores_liver_anomaly_test is not None:
        scores.extend(scores_liver_anomaly_test)
        save_to_excel(scores_liver_anomaly_test, liver_anomaly_test_im_names, liver_anomaly_test_labels, os.path.join(output_path, cfg.csv_liver_tissue_anomalies))
        save_images_from_excel(os.path.join(output_path, cfg.csv_liver_tissue_anomalies), output_path, cfg.save_n_FN, 'FN')

if non_liver_test_data_loader:
    logging.info('-----------anomaly detection in different to liver tissues---------------')
    scores_non_liver_test, non_liver_test_im_names, non_liver_test_labels = anomaly_detection(clf, model, non_liver_test_data_loader, dev, feat_container=features_to_visualize, cache_path=output_path)
...
if (scores_normal_test is not None) and (scores_liver_anomaly_test is not None):
    with open(os.path.join(output_path, 'anomaly_detector_results.txt'), 'w') as fh:
...
if scores:
    ...
    TSNE_visualization(features_to_visualize_sampled['features'], features_to_visualize_sampled['labels'], scores=scores_sampled, save_path=output_path, colormap='tab20')
